...
export const UniCompRenderer: React.FC<UniCompRendererProps> = ({
  ...
  const selectionHasParam = useCallback((paramType: 'st' | 'sp' | 'w' | 'rotate' | 'colorGroup'): boolean => {
    ...
    return selectionSet.some(idx => {
      ...
      if (!sym.history || sym.history.length === 0) {
        ...
        if (paramType === 'rotate') => sym.rotate !== undefined;
        if (paramType === 'colorGroup') => !!(sym.color || sym.background || sym.strokeColor);
      }
      if (sym.history && sym.history.length > 0) {
        ...
        if (paramType === 'rotate') => !!base.rotate;
        if (paramType === 'colorGroup') => !!base.colorGroup;
      }
      return false;
    });
  }, [spec, selectionSet]);
  ...
  const handleMouseMove = useCallback((e: MouseEvent | TouchEvent) => {
    ...
    if (isEditing === 'move') {
      ...
      const width0 = x2 - x1 + 1;
      const height0 = y2 - y1 + 1;

      // Пропорциональное масштабирование по сумме движений
      const scaleFactor = 1 + (dx + dy) / (initCellSize * Math.min(width0, height0));
      const newWidth = Math.max(1, Math.round(width0 * scaleFactor));
      const newHeight = Math.max(1, Math.round(height0 * scaleFactor));

      const newX1 = x1;
      const newY1 = y1;
      const newX2 = x1 + newWidth - 1;
      const newY2 = y1 + newHeight - 1;

      newSpec = transformGroupScale(newSpec, selectionSet, newX1, newY1, newX2, newY2);
