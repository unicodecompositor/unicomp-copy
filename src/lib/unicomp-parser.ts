...
export function transformGroupMove(
  ...
  let newSpec = JSON.parse(JSON.stringify(spec)) as UniCompSpec;
  const { gridWidth } = newSpec;

  // Применяем смещение к выбранным слоям
  for (const idx of indices) {
    const sym = newSpec.symbols[idx];
    if (!sym) continue;

    const d = sym.d ?? { start: sym.start, end: sym.end };
    const startXY = indexToXY(d.start, gridWidth);
    const endXY = indexToXY(d.end, gridWidth);
    const newStartXY = { x: startXY.x + deltaX, y: startXY.y + deltaY };
    const newEndXY = { x: endXY.x + deltaX, y: endXY.y + deltaY };

    const newStart = xyToIndex(newStartXY.x, newStartXY.y, gridWidth);
    const newEnd = xyToIndex(newEndXY.x, newEndXY.y, gridWidth);
    sym.d = { start: newStart, end: newEnd };
    sym.start = newStart;
    sym.end = newEnd;
  }

  // Нормализуем всю сцену
  newSpec = normalizeSpec(newSpec);

  // Записываем историю для каждого перемещённого слоя: po
  for (const idx of indices) {
    const sym = newSpec.symbols[idx];
...
/**
 * Масштабирует группу слоёв, задавая новые координаты start и end.
 */
export function transformGroupScale(
  ...
  for (const idx of indices) {
    ...
    const newStart = xyToIndex(newStartX, newStartY, gridWidth);
    const newEnd = xyToIndex(newEndX, newEndY, gridWidth);
    sym.d = { start: newStart, end: newEnd };
    sym.start = newStart;
    sym.end = newEnd;
  }
...
export function parseMultiLine(input: string): MultiLineParseResult {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    blocks.push({ lineNumber: i + 1, raw: line, result: result.success ? result : null });
...
export function stringifySpec(spec: UniCompSpec, mode: 'editor' | 'export' = 'editor'): string {
  ...
  if (mode === 'editor' && spec.grid.pg) gridParams.push(`pg=${spec.grid.pg.start}-${spec.grid.pg.end}`);
  if (mode === 'editor' && spec.grid.po) gridParams.push(`po=${spec.grid.po.start}-${spec.grid.po.end}`);
  ...
  for (const sym of spec.symbols) {
    ...
    if (mode === 'editor' && sym.pg) params.push(`pg=${sym.pg.start}-${sym.pg.end}`);
    if (mode === 'editor' && sym.po) params.push(`po=${sym.po.start}-${sym.po.end}`);
...
export function exportToUniStr(spec: UniCompSpec): string {
  return `START${spec.symbols.map(s => `DELTA v=${s.v};d=${s.d?.start ?? s.start}-${s.d?.end ?? s.end}`).join(';')}STOP`;
}
...
export function exportToUniNode(spec: UniCompSpec): string {
  return spec.symbols.map(s => `#${s.id || 'node'}[d=${s.d?.start ?? s.start}-${s.d?.end ?? s.end}]`).join(';');
}
...
export {
  type Vec2, type Vec3, type Vec4, type Bounds,
  type TransformVector, type LayerStyles, type GridStyles, type Space3D,
  type HistoryStep, type KeyframeStep,
  type SymbolSpec, type GridSpec, type UniCompSpec,
  type BakedSymbol, type BakedSpec,
  type ParseResult, type MultiLineParseResult, type MultiLineBlock, type ErrorLine,
  type UniFormat, type PlayState, type DeltaOp,
  SECURITY_LIMITS,
  PLAY_STATES,
  TokenType,
  parseBounds, formatBounds, parseVec2, parseVec3, parseVec4, parseAngleForce,
  getRect, normalizeDegrees,
  parsePlayState, playStateToString,
  computeDFromPgPo, computePoFromD, computeGridSize,
  bakeForExport, unbakeForEditor,
  resolveHistory, appendTransformToHistory, undoLastHistoryParam,
  parseUniComp, parseMultiLine, stringifySpec,
  exportToUniComp, exportToUniPNG, exportToUniGIF, exportToUniMPG,
  exportToUniStr, exportToUni3DS, exportToUniLib, exportToUniAI,
  exportToUniAsc, exportToUniNode, exportToUniSym,
  detectFormat,
};